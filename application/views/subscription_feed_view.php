...
<ul class="list">
    ...
    <?php if (isset($results)) : foreach ($results as $row) : ?>
        ...
        <li class="clearfix">
            ...
            <a class="title" href ="<?=base_url()?>article/<?=$row->id?>"><?=$row->title;?></a>
            <span class="date"><?="By ". anchor("feed/".$row->user_id, $this->blog_model->userFromID($row->user_id,'firstname')." ".$this->blog_model->userFromID($row->user_id,'lastname'))." on ".date("Y-m-d", $row->date)." at ".date("g:ia", $row->date); ?></span>
            <p class="summary"><?=htmlspecialchars_decode($row->preview);?></p>
